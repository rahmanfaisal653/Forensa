...
// Settings are stored in localStorage (per-user, per-browser).
// The actual API calls go through the local backend proxy (server.js)
// to avoid CORS and keep the key out of the browser bundle.
//
// Fallback: when no custom provider is configured, the app uses the
// original Gemini SDK path (GEMINI_API_KEY from env) — see geminiService.
...
/** Ask our backend proxy to generate a chat completion (SSE stream). */
